...
export default function CompetitionPanel(props: Props) {
	...
	const [hoveredMult, setHoveredMult] = createSignal<MultiplierStatus | null>(
		null,
	);
	...
	const [allQsoData] = createResource(fetchKey, async (key) => {
		if (!key) return [];
		const rawList = await invoke<string[]>("get_all_qso_data", {
			logbookId: key.logbook_id,
		});
	...
	return (
		...
		>
			...
			<div class="competition-summary">
				...
				<div class="competition-stat">
					<span class="competition-stat-value">
						{scores()?.total_multipliers_worked ?? 0}/
						{scores()?.total_multipliers_possible ?? 0}
					</span>
					<span class="competition-stat-label">
						{props.competition.multipliers.label}
					</span>
				</div>
				<div class="competition-stat">
					<span class="competition-stat-value">
						{scores()?.completion_pct ?? 0}%
					</span>
			...
			<div class="competition-grid-wrapper">
				<div class="competition-grid-title">
					{props.competition.multipliers.label} Progress
				</div>
				<div
					class="competition-grid"
					style={{
						"grid-template-columns": `repeat(${props.competition.multipliers.grid_columns}, 1fr)`,
					}}
				>
						...
						{(mult) => (
							...
							>
								...
								<Show when={mult.worked}>
									<span class="competition-cell-count">
										{mult.contacts.length}
									</span>
								</Show>
							</div>
						)}
			...
			<Show
				when={
					scores()?.band_breakdown &&
					Object.keys(scores()!.band_breakdown!).length > 0
				}
			>
				<div class="competition-band-breakdown">
					...
					<div class="competition-band-chips">
						<For
							each={Object.entries(scores()!.band_breakdown!).sort(
								(a, b) => b[1] - a[1],
							)}
						>
							{([band, count]) => (
			...
			<Show when={hoveredMult()}>
				...
				>
					...
					<div class="competition-tooltip-count">
						{hoveredMult()!.contacts.length} contact
						{hoveredMult()!.contacts.length !== 1 ? "s" : ""}
					</div>
