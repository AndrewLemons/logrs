		...
		<div class="station-bar">
			<div class="station-bar-context">
				<select class="station-bar-select" onChange={onProfileChange}>
					<option value="" selected={!activeProfile()}>
						Select profile...
					</option>
					<For each={profiles()}>
						{(p) => (
							<option value={p.id} selected={p.id === activeProfile()?.id}>
								{p.callsign}
							</option>
						)}
					</For>
				...
				<select class="station-bar-select" onChange={onLogbookChange}>
					<option value="" selected={!activeLogbook()}>
						Select logbook...
					</option>
					<For each={logbooks()}>
						{(lb) => (
							<option value={lb.id} selected={lb.id === activeLogbook()?.id}>
								{lb.name}
							</option>
						)}
					</For>
