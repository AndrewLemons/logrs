...
export default function LogTable(props: LogTableProps) {
  ...
  const totalPages = () => Math.max(1, Math.ceil((props.logData?.total ?? 0) / 100));
  ...
  async function deleteSelected() {
    ...
    if (!(await confirm(`Delete ${ids.length} QSO(s)?`, { title: "Delete QSOs", kind: "warning" }))) return;
    try {
              ...
              <tr>
                <th style={{ width: "32px" }}>
                  <input
                    type="checkbox"
                    class="checkbox"
                    checked={selected().size === (props.logData?.qsos.length ?? 0) && selected().size > 0}
                    onChange={toggleSelectAll}
                  />
                </th>
                <th style={{ width: "32px" }} />
                <th onClick={() => props.onToggleSort("datetime")}>Date/Time{sortIndicator("datetime")}</th>
                <For each={props.tableFields}>
                  {(field) => (
                    <th onClick={() => props.onToggleSort(field.id)}>
                      {field.label}{sortIndicator(field.id)}
                    </th>
                  )}
                </For>
              </tr>
                ...
                {(qso) => {
                  ...
                  return (
                    <tr>
                      ...
                      <td class="data-table-date">{formatDateTime(qso.datetime)}</td>
                      <For each={props.tableFields}>
                        {(field) => (
                          <td
                            class={field.id === "callsign" ? "mono data-table-callsign" : field.id === "notes" ? "data-table-notes" : ""}
                          >
                            {data()[field.id] ?? ""}
                          </td>
                        )}
