...
import type { TemplateDef, TemplateField, CallsignInfo, PotaSpotResult, Qso } from "../../types";
...
interface QsoEntryFormProps {
  ...
  setQsoValues: (fn: (prev: Record<string, string>) => Record<string, string>) => void;
  onSaved: () => void;
  totalQsos: number;
}

export default function QsoEntryForm(props: QsoEntryFormProps) {
  const [lookupStatus, setLookupStatus] = createSignal<"idle" | "loading" | "found" | "not-found">("idle");
  ...
  async function patchQsoBg(savedQso: Qso, patches: Record<string, string>): Promise<void> {
    ...
    try {
      await invoke("update_qso", {
        qso: { ...savedQso, data_json: JSON.stringify({ ...data, ...updates }) },
      });
      props.onSaved();
    } catch {
  ...
  function doLookup() {
    ...
    pendingCallsignInfo = invoke<CallsignInfo | null>("lookup_callsign", { callsign: cs })
      .catch(() => null);
    ...
    pendingCallsignInfo.then((info) => {
      ...
      if (info) {
        ...
        if (info.name && (autoFilledFields.has("name") || !props.qsoValues.name))
    ...
    const hasTheirPark = props.templateDef?.fields.some((f) => f.id === "their_park");
    if (hasTheirPark) {
      pendingPotaSpot = invoke<PotaSpotResult | null>("lookup_pota_activator", { callsign: cs })
        .catch(() => null);

      pendingPotaSpot.then((spot) => {
        const currentCs = (props.qsoValues.callsign ?? "").trim().toUpperCase();
        if (currentCs !== cs) return;
        if (spot?.reference && (autoFilledFields.has("their_park") || !props.qsoValues.their_park)) {
    ...
    if (logbook && band) {
      invoke<boolean>("check_duplicate", { logbookId: logbook.id, callsign: cs, band })
  ...
  async function saveQso(e?: Event) {
    ...
    try {
      ...
      const callsignPromise = pendingCallsignInfo
        ?? invoke<CallsignInfo | null>("lookup_callsign", { callsign: cs }).catch(() => null);

      const hasTheirPark = props.templateDef?.fields.some((f) => f.id === "their_park");
      const potaPromise: Promise<PotaSpotResult | null> = hasTheirPark
        ? (pendingPotaSpot ?? invoke<PotaSpotResult | null>("lookup_pota_activator", { callsign: cs }).catch(() => null))
    ...
    <form class="qso-entry" onSubmit={saveQso}>
      ...
      <Show when={isDuplicate()}>
        <div class="duplicate-warning">
          <AlertTriangle size={14} /> Possible duplicate — same callsign + band in the last 30 minutes
        </div>
      </Show>
