...
export default function Logbooks() {
	...
	const [templates] = createResource(() => invoke<Template[]>("get_templates"));
	const [profiles] = createResource(() => invoke<Profile[]>("get_profiles"));

	// Listen for external changes
	let unsub: (() => void) | undefined;
	onMount(() => {
		unsub = on(Events.LOGBOOKS_CHANGED, refetch);
	});
	onCleanup(() => unsub?.());
	...
	async function deleteLogbook(id: number, lbName: string) {
		if (
			!(await confirm(`Delete logbook "${lbName}"? All QSOs will be deleted.`, {
	...
	return (
		<div>
			...
			<Show when={!logbooks()?.length}>
				<EmptyState
					title="No Logbooks"
					description="Create your first logbook to start logging contacts."
				>
					<button class="btn btn-primary" onClick={() => setShowCreate(true)}>
						+ Create Logbook
					</button>
					...
					{(lb) => (
						<Card
							...
							headerActions={
								<button
									class="btn-icon"
									onClick={() => deleteLogbook(lb.id, lb.name)}
									title="Delete"
								>
									<Trash2 size={14} />
								</button>
							...
							actions={
								<>
									<button
										...
										onClick={() => setActiveLogbookAction(lb)}
									>
										Set Active
									</button>
									<button
										class="btn btn-secondary btn-sm"
										onClick={() => openLogbook(lb)}
									>
										Open
									</button>
									<button
										class="btn btn-secondary btn-sm"
										onClick={() => importAdif(lb.id)}
									>
										Import ADIF
									</button>
								</>
			...
			>
				...
				<div class="modal-footer">
					...
					<button class="btn btn-primary" onClick={createLogbook}>
						Create
					</button>
				</div>
			</Modal>
