...
export default function Workspace() {
	...
	const stationFields = () =>
		templateDef()?.fields.filter((f) => f.category === "station") ?? [];
	const qsoFields = () =>
		templateDef()?.fields.filter((f) => f.category === "qso") ?? [];
	const tableFields = () =>
		templateDef()?.fields.filter((f) => f.show_in_table) ?? [];
	...
	// --- Activity panel ---
	const [activeTab, setActiveTab] = createSignal<"log" | "map" | "competition">(
		"log",
	);
	...
	return (
		<div class="workspace">
			...
			<Show when={!activeProfile() || !activeLogbook()}>
				<EmptyState
					title={
						!activeProfile() ? "No Profile Selected" : "No Logbook Selected"
					}
			...
			<Show when={activeProfile() && activeLogbook()}>
				...
				<div class="activity-panel">
					...
					<QsoMap logData={logData()} visible={activeTab() === "map"} />
