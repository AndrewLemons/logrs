...
const emptyProfile: CreateProfile = {
  callsign: "", name: "", grid: "", default_power: "",
  default_band: "", default_mode: "", default_park: "",
  default_summit: "", station_description: "",
};
...
export default function Profiles() {
  ...
  onMount(() => { unsub = on(Events.PROFILES_CHANGED, refetch); });
  onCleanup(() => unsub?.());
  ...
  async function saveProfile() {
    ...
    try {
      if (editing()) {
        ...
        await invoke("create_profile", { profile: { ...f, callsign: f.callsign.toUpperCase() } });
  ...
  async function deleteProfile(id: number, callsign: string) {
    if (!(await confirm(`Delete profile "${callsign}"?`, { title: "Delete Profile", kind: "warning" }))) return;
    try {
  ...
  return (
    <div>
      <PageHeader title="Profiles">
        <button class="btn btn-primary" onClick={openCreate}>+ New Profile</button>
      </PageHeader>

      <Show when={!profiles()?.length}>
        <EmptyState title="No Profiles" description="Create a profile with your callsign to get started.">
          <button class="btn btn-primary" onClick={openCreate}>+ Create Profile</button>
        </EmptyState>
      </Show>
            ...
            <Card
              ...
              headerActions={
                <div class="card-actions">
                  <button class="btn-icon" onClick={() => openEdit(p)} title="Edit"><Pencil size={14} /></button>
                  <button class="btn-icon" onClick={() => deleteProfile(p.id, p.callsign)} title="Delete"><Trash2 size={14} /></button>
                </div>
              }
              actions={
                <button class="btn btn-primary btn-sm" onClick={() => activateProfile(p)}>
                  Set Active
                </button>
              }
            >
      ...
      <Modal open={showModal()} onClose={() => setShowModal(false)}
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <input class="form-input callsign" type="text" value={form().callsign}
              onInput={(e) => updateField("callsign", e.currentTarget.value.toUpperCase())}
              placeholder="W1ABC" />
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input class="form-input" type="text" value={form().grid}
              onInput={(e) => updateField("grid", e.currentTarget.value.toUpperCase())}
              placeholder="FN31pr" />
          </div>
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">Default Power (W)</label>
              <input class="form-input" type="text" value={form().default_power}
                onInput={(e) => updateField("default_power", e.currentTarget.value)} placeholder="100" />
            </div>
            <div class="form-group">
              <label class="form-label">Default Band</label>
              <select class="form-select" value={form().default_band}
                onChange={(e) => updateField("default_band", e.currentTarget.value)}>
                <option value="">—</option>
            ...
            <div class="form-group">
              ...
              <select class="form-select" value={form().default_mode}
                onChange={(e) => updateField("default_mode", e.currentTarget.value)}>
                <option value="">—</option>
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input class="form-input" type="text" value={form().default_park}
                onInput={(e) => updateField("default_park", e.currentTarget.value)} placeholder="K-1234" />
            </div>
            <div class="form-group">
              <label class="form-label">Default Summit</label>
              <input class="form-input" type="text" value={form().default_summit}
                onInput={(e) => updateField("default_summit", e.currentTarget.value)} placeholder="W4G/NG-001" />
            </div>
          </div>
          <div class="form-group">
            <label class="form-label">Station Description</label>
            <input class="form-input" type="text" value={form().station_description}
              onInput={(e) => updateField("station_description", e.currentTarget.value)}
              placeholder="IC-7300 + EFHW" />
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-secondary" onClick={() => setShowModal(false)}>Cancel</button>
