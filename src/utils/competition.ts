...
export const CQ_ZONES: MultiplierValue[] = Array.from(
	{ length: 40 },
	(_, i) => ({
		id: String(i + 1),
		label: `Zone ${i + 1}`,
...
export const ITU_ZONES: MultiplierValue[] = Array.from(
	{ length: 90 },
	(_, i) => ({
		id: String(i + 1),
		label: `Zone ${i + 1}`,
...
export function computeScores(
	...
	const multiplierPoints =
		totalMultipliersWorked * scoring.bonus_per_new_multiplier;
	const totalPoints = rawPoints + multiplierPoints;
	const completionPct =
		totalMultipliersPossible > 0
			? Math.round(
					((statusMap.size > 0
						? [...statusMap.values()].filter((s) => s.worked).length
						: 0) /
						totalMultipliersPossible) *
						100,
				)
			: 0;
