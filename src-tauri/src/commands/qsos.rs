...
pub fn create_qso(db: State<DbState>, qso: CreateQso) -> Result<Qso, String> {
    ...
    let mut data: serde_json::Value = serde_json::from_str(&qso.data_json)
        .map_err(|e| format!("Invalid QSO data: {}", e))?;
...
pub fn update_qso(db: State<DbState>, qso: Qso) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let _: serde_json::Value = serde_json::from_str(&qso.data_json)
        .map_err(|e| format!("Invalid QSO data: {}", e))?;
    conn.execute(
